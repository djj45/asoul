...
如果电脑的显示缩放不是100%，<del>Aegisub的取色器就会出现问题，取色的区域是取色器上方的区域而不是取色器触碰到的那个点</del>d，十六进制颜色写在红框处
...
aegisub不能导入LRC格式，要先转换成SRT格式（ASS格式也可以），用转换网站转换

http://www.lrccon.com/convert.php

或者可以用ffmpeg转换，不过ffmpeg转换后最后一句的结束时间变成了无穷大，导致导入aegisub出错，可以用记事本打开SRT文件，把最后一句的结束时间改为开始时间加上10秒（根据实际情况延长或者缩短）

```
ffmpeg -i input.lrc output.srt
```
